...
export default function SourcesPage() {
 ...
 // Stable task ID per source file (prevents duplicates)
 const taskIdForSource = (name: string) => `analysis:${name}`;

 // Check if this source is currently being analyzed
 const isAnalyzing = (src: any) => {
 return isTaskRunning(taskIdForSource(src.name));
 };

 // Run a real analysis on the clicked source globally
 ...
 disabled={uploading}
 className="bg-[var(--primary)] hover:bg-[var(--primary)] disabled:opacity-50 text-white font-bold rounded-md px-4 py-2 text-xs border-none flex items-center gap-2 cursor-pointer transition-colors"
 >
 ...
 <p className="text-2xl font-bold text-[var(--primary)]">{humanSize(totalSize)}</p>
 </div>
 <div className="bg-[var(--bg-card)] rounded-xl p-4">
 ...
 <div className="w-10 h-10 rounded-lg bg-[var(--primary)] border border-[var(--primary)] flex items-center justify-center">
 ...
 disabled={isAnalyzing(src)}
 className="text-[10px] font-bold text-[var(--primary)] hover:text-[var(--primary)] disabled:opacity-50 transition-colors uppercase tracking-widest flex items-center gap-1 cursor-pointer bg-transparent border-none"
 >
 {isAnalyzing(src) ? (
 ...
 <button 
 onClick={() => fileInputRef.current?.click()}
 className="bg-[var(--bg-card)] border border-dashed border-[var(--border)] rounded-xl p-6 flex flex-col items-center justify-center h-[200px] hover:border-[var(--primary)] hover:bg-[var(--primary)] transition-all cursor-pointer group"
 >
 <div className="w-12 h-12 rounded-full bg-[var(--bg-surface)] flex items-center justify-center mb-3 group-hover:bg-[var(--primary)] transition-colors">
 <Plus size={24} className="text-[var(--text-muted)] group-hover:text-[var(--primary)] transition-colors" />
 </div>
 ...
 className="text-[10px] font-bold text-[var(--primary)] hover:text-[var(--primary)] transition-colors uppercase tracking-widest flex items-center gap-1 cursor-pointer bg-transparent border-none"
 >
 Configure
 </button>
 </div>
 ...
 <div className="mx-6 mt-4 p-3 bg-[var(--primary)] border border-[var(--primary)] rounded-lg flex items-center gap-3">
 ...
 className={`p-3 flex items-center justify-between cursor-pointer transition-colors ${selectedK8sPod?.name === pod.name ? 'bg-[var(--primary)]' : 'hover:bg-[var(--bg-surface-hover)]'}`}
 >
 ...
 className="bg-[var(--primary)] hover:bg-[var(--primary)] disabled:opacity-50 text-white font-bold rounded-md px-4 py-2 text-xs border-none cursor-pointer flex items-center gap-2"
 >
 ...
 className={`p-3 flex items-center justify-between cursor-pointer transition-colors ${selectedAwsGroup?.name === group.name ? 'bg-[var(--primary)]' : 'hover:bg-[var(--bg-surface-hover)]'}`}
 >
 ...
 className={`p-3 flex items-center justify-between cursor-pointer transition-colors ${selectedDockerContainer?.id === container.id ? 'bg-[var(--primary)]' : 'hover:bg-[var(--bg-surface-hover)]'}`}
 >
