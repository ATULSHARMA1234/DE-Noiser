      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="bg-fuchsia-600 disabled:bg-[var(--bg-track)] disabled:text-[var(--text-dimmed)] hover:bg-fuchsia-500 text-white font-medium rounded-md px-4 py-2 text-xs flex items-center gap-2 cursor-pointer transition-colors border-none"
          >
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-transparent">
            {isLoading ? (
              ...
              <tr key={run.id} className={`hover:bg-[var(--bg-surface-hover)] transition-colors rounded-lg ${selectedRuns.includes(run.id) ? 'bg-fuchsia-500/10' : ''}`}>
                <td className="p-5 text-center">
                  <input 
                    type="checkbox" 
                    checked={selectedRuns.includes(run.id)} 
                    onChange={() => toggleSelection(run.id)}
                    className="w-4 h-4 rounded bg-[var(--bg-input)] border-[var(--border)] accent-fuchsia-500 cursor-pointer"
                  />
                </td>
                <td className="p-5">
                ...
                <td className="p-5">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[var(--text-secondary)]"><strong className="text-fuchsia-400">{run.cluster_count}</strong> clusters</span>
