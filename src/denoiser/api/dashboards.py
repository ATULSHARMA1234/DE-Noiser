...
import datetime
import random
from denoiser.storage.db import get_db, Dashboard as DBDashboard
from denoiser.api.auth import get_current_user, require_role, User
from denoiser.dashboards.models import DashboardCreateSchema, DashboardUpdateSchema, DashboardSchema
...
def list_dashboards(db: Session = Depends(get_db), current_user: User = Depends(require_role(["VIEWER", "ANALYST", "ADMIN"]))):
    ...
    dashboards = db.query(DBDashboard).filter(
        ...
        ((DBDashboard.is_shared == True) | (DBDashboard.user_id == current_user.id))
    ).all()
    return dashboards
