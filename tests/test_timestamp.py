...
class TestTimestampExtractor:
    ...
    def test_extract_iso8601_utc(self, extractor):
        ...
        expected = int(datetime(2026, 5, 22, 23, 25, 51, microsecond=123000, tzinfo=UTC).timestamp() * 1000)
    ...
    def test_extract_iso8601_slash_and_comma(self, extractor):
        ...
        expected = int(datetime(2026, 5, 22, 23, 25, 51, microsecond=456000, tzinfo=UTC).timestamp() * 1000)
        assert extractor.extract(log) == expected
    ...
    def test_extract_iso8601_timezone_offset(self, extractor):
        ...
        expected = int(datetime(2026, 5, 22, 17, 55, 51, microsecond=123000, tzinfo=UTC).timestamp() * 1000)
        assert extractor.extract(log) == expected
    ...
    def test_extract_syslog_basic(self, extractor):
        ...
        expected = int(datetime(year, 5, 22, 23, 25, 51, tzinfo=UTC).timestamp() * 1000)
        assert extractor.extract(log) == expected
    ...
    def test_extract_syslog_single_digit_day(self, extractor):
        ...
        year = datetime.now(UTC).year
        expected = int(datetime(year, 5, 2, 23, 25, 51, tzinfo=UTC).timestamp() * 1000)
        assert extractor.extract(log) == expected
    ...
    def test_extract_syslog_year_fallback(self, extractor):
        ...
        parsed_dt = datetime.fromtimestamp(parsed / 1000, UTC)
